...
sub vcl_recv {
    ...
    # Redirect http to https for active domains
    if (client.ip != "127.0.0.1" && req.http.host ~ "radiumthemes.com") {
        set req.http.x-redir = "https://radiumthemes.com" + req.url;
        return(synth(850, ""));
    }
    ...
    if (req.url ~ "\.(gif|jp(e?)g|swf|css|js|map|flv|mp3|mp4|pdf|ico|png|tif|tiff|ttf|mp3|htm|html)(\?.*|)$") {
		unset req.http.cookie;
	}
	...
	if (req.http.Accept-Encoding) {
		...
		if (req.url ~ "\.(jp(e?)g|png|gif|gz|tgz|bz2|tbz|mp3|ogg)$") {
